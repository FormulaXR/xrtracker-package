...
namespace IV.FormulaTracker
{
	public class TrackerBackgroundRenderer : MonoBehaviour
	{
		...
		private void SetUp()
		{
			...
			if (_mainCamera == null)
			{
				Debug.LogError("[FormulaBackgroundRenderer] No main camera found! Ensure FormulaTrackingManager has a camera assigned.", this);
				return;
			}
		...
		private void ConfigureRenderPipeline()
		{
			...
			if (IsURP() && _mainCamera.targetTexture == null && TryConfigureURPOverlay())
			{
				...
				Debug.Log("[FormulaBackgroundRenderer] Configured for URP (Base + Overlay)");
				return;
			}

			// Depth-based for Built-in, HDRP, or URP with RenderTexture
			_mainCamera.clearFlags = CameraClearFlags.Depth;
			Debug.Log("[FormulaBackgroundRenderer] Configured with depth-based rendering");
		}
