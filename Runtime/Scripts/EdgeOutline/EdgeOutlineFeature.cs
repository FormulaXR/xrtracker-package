...
#pragma warning restore CS0618
		...
		class EdgeOutlinePass : ScriptableRenderPass
		{
			...
			private const int STENCIL_NOT_EQUAL = 6;
			private const int STENCIL_ALWAYS = 8;
					...
					builder.SetRenderFunc(static (PassData data, UnsafeGraphContext context) =>
					{
						...
						if (depthRT != null)
						{
							...
							foreach (var outline in EdgeOutlineRenderer.Instances)
							{
								...
								// Fast path: single pre-combined occlusion mesh in outline-local space.
								Mesh occlusionMesh = outline.OcclusionMesh;
								if (occlusionMesh != null && occlusionMesh.vertexCount > 0)
								{
									cmd.DrawMesh(occlusionMesh, outline.transform.localToWorldMatrix,
									             data.OcclusionMat, 0, 0);
									continue;
								}

								// Fallback: per-submesh draw (when no combined mesh is available).
						...
						foreach (var outline in EdgeOutlineRenderer.Instances)
						{
							...
							Mesh creaseMesh = outline.CreaseMesh;
							Mesh edgeMesh = outline.EdgeMesh;
							bool hasCrease = creaseMesh != null && creaseMesh.vertexCount > 0;
							bool hasEdge = edgeMesh != null && edgeMesh.vertexCount > 0;
							if (!hasCrease && !hasEdge)
								continue;

							data.EdgeMat.SetInt(StencilCompProp, outline.ShowInternalEdges ? STENCIL_ALWAYS : STENCIL_NOT_EQUAL);
							...
							if (hasCrease)
								cmd.DrawMesh(creaseMesh, matrix, data.EdgeMat, 0, 0, PropertyBlock);
							if (hasEdge)
