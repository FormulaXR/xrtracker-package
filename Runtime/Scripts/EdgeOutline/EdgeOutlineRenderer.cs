...
namespace IV.FormulaTracker
{
	...
	public abstract class EdgeOutlineRenderer : MonoBehaviour
	{
		...
		[Header("Crease Detection")]
		[Tooltip("Enable crease edge detection. When disabled, only silhouette (contour) edges are drawn.")]
		[SerializeField] bool _enableCreaseEdges = true;
		...
		[Header("Rendering")]
		[Tooltip("Draw mesh to depth buffer first so edges behind the object are hidden.")]
		[SerializeField] bool _enableOcclusion = true;
		...
		struct NativeEdgeData
		{
			public int v0Idx, v1Idx;
			public Vector3 n0, n1;
			public byte isCrease; // 1 = crease, 0 = potential silhouette
		}
		...
		public bool EnableCreaseEdges
		{
			...
			set { _enableCreaseEdges = value; _dirty = true; }
		}
		...
		{
			if (Mathf.Abs(_builtAngle - _creaseAngle) > 0.1f || _builtCrease != _enableCreaseEdges)
				_dirty = true;
		}
		...
		void BuildEdges()
		{
			...
			_builtCrease = _enableCreaseEdges;
			_dirty = false;
			_meshFilters = null;
			_renderers = null;
			...
			{
				var e = _edges[i];
				if (e.isCrease && !_enableCreaseEdges) continue;
				_nativeEdges[nativeCount++] = new NativeEdgeData
				{
			...
			if (nativeCount < edgeCount)
			{
				var trimmed = new NativeArray<NativeEdgeData>(nativeCount, Allocator.Persistent);
				NativeArray<NativeEdgeData>.Copy(_nativeEdges, trimmed, nativeCount);
				_nativeEdges.Dispose();
				_nativeEdges = trimmed;
			}

			edgeCount = nativeCount;
			...
			public void Execute()
			{
				...
				for (int i = 0; i < edges.Length; i++)
				{
					...
					if (e.isCrease == 0)
					{
						...
						float d1 = e.n1.x * viewDir.x + e.n1.y * viewDir.y + e.n1.z * viewDir.z;
						if (d0 * d1 > 0f) continue;
					}
