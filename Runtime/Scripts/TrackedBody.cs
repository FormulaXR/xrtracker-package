...
namespace IV.FormulaTracker
{
	...
	public class TrackedBody : MonoBehaviour
	{
		...
		{
			// Determine which viewpoint to use
			Transform viewpoint = _initialPoseSource == InitialPoseSource.Viewpoint && _initialViewpoint != null
				? _initialViewpoint
				: CameraTransform;

			// Compute pose relative to viewpoint
			Vector3 relativePosition = viewpoint.InverseTransformPoint(transform.position);
			Quaternion relativeRotation = Quaternion.Inverse(viewpoint.rotation) * transform.rotation;

			_initialPose = ConversionUtils.GetConvertedPose(relativePosition, relativeRotation);
		}
		...
		public void SetInitialPose(Transform viewpoint)
		{
			...
			Vector3 relativePosition = vp.InverseTransformPoint(transform.position);
			Quaternion relativeRotation = Quaternion.Inverse(vp.rotation) * transform.rotation;
			_initialPose = ConversionUtils.GetConvertedPose(relativePosition, relativeRotation);
		...
		{
			_initialPose = ConversionUtils.GetConvertedPose(position, rotation);
			ApplyInitialPoseToNative();
		}
