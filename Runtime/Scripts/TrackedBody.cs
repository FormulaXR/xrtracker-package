...
namespace IV.FormulaTracker
{
	...
	public class TrackedBody : MonoBehaviour
	{
		...
		[SerializeField]
		private TrackingMethod _trackingMethod = TrackingMethod.Edge;
		...
		/// Used by quality gating to react immediately to bad optimizer results.
		/// </summary>
		public float InstantQuality
		{
			get
			{
				float quality = InstantSilhouetteQuality;
				if (_enableDepthTracking)
					quality = Mathf.Max(quality, InstantDepthQuality);
				if (_enableEdgeTracking)
					quality = Mathf.Max(quality, InstantEdgeQuality);
				return quality;
			}
		}

		public float InstantEdgeQuality =>
			_lastStatus.n_total_edge_sites > 0
				? ComputeEdgeQuality((float)_lastStatus.n_valid_edge_points / _lastStatus.n_total_edge_sites)
				: 0f;

		public float InstantDepthQuality =>
			ComputeDepthQuality(_lastStatus.mean_correspondence_distance);

		public float InstantSilhouetteQuality =>
			ComputeSilhouetteQuality(_lastStatus.histogram_discriminability);

		private float ComputeEdgeQuality(float coverage)
		{
			if (!_enableEdgeTracking || !_lastStatus.HasEdgeModality)
				return 0;
			return Mathf.InverseLerp(0f, TrackerDefaults.EDGE_QUALITY_COVERAGE_MAX, coverage);
		}

		private float ComputeDepthQuality(float correspondenceDistance)
		{
			...
			return 1f - Mathf.Clamp01(correspondenceDistance / _depthTracking.DistanceTolerance);
		}

		private float ComputeSilhouetteQuality(float histogramDiscriminability)
		{
			if (Visibility <= 0)
				return 0;
			float globalPeak = TrackedBodyManager.Instance?.GlobalHistogramPeak ?? TrackerDefaults.HISTOGRAM_GOOD;
			return Mathf.InverseLerp(TrackerDefaults.HISTOGRAM_BAD, globalPeak, histogramDiscriminability);
		}
		...
		public float NiceQualityThreshold => _useCustomNiceQuality
			? _customNiceQualityThreshold
			: _enableEdgeTracking
				? TrackerDefaults.EDGE_NICE_QUALITY_THRESHOLD
				: TrackerDefaults.NICE_QUALITY_THRESHOLD;
		...
		public virtual void BeforeTrackingStep()
		{
			...
			// Convert detector pose from world to anchor space to match.
			var fusion = TrackedBodyManager.Instance?.ARPoseFusion;
			if (fusion != null && fusion.HasReliableAnchor)
			{
				if (fusion.TryWorldToAnchor(worldPos, worldRot, out var anchorPos, out var anchorRot))
				{
					var anchorPose = ConversionUtils.GetConvertedPose(anchorPos, anchorRot);
					FTBridge.FT_SetDetectorPose(_bodyId, ref anchorPose);
					FTBridge.FT_SetBodyPose(_bodyId, ref anchorPose);
					return;
				}
			}
#endif
		...
		/// <summary>
		/// Apply detector pose directly to Unity Transform. Called every render frame
		/// by TrackedBodyManager so the body follows the camera smoothly during detection,
		/// independent of tracking step timing.
		/// </summary>
		internal void ApplyDetectorPose()
		{
			Transform cam = CameraTransform;
			if (cam == null) return;
			SetWorldPose(
				cam.TransformPoint(_initialCameraRelativePosition),
				cam.rotation * _initialCameraRelativeRotation);
		}
