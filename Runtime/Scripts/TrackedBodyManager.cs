...
namespace IV.FormulaTracker
{
	...
	public class TrackedBodyManager : MonoBehaviour
	{
		...
		private static float NiceQuality(TrackedBody body) => body.EnableEdgeTracking ? TrackerDefaults.EDGE_NICE_QUALITY_THRESHOLD : TrackerDefaults.NICE_QUALITY_THRESHOLD;
		private static int StationaryFramesToRecover => TrackerDefaults.STATIONARY_FRAMES_TO_RECOVER;
		...
		/// <summary>
		/// AR stationary mode phases.
		/// </summary>
		private enum ARPhase
		{
			/// <summary>Not in AR mode.</summary>
			None,

			/// <summary>Applying native pose for STABILIZATION_FRAMES to establish anchor.</summary>
			Stabilizing,

			/// <summary>SLAM holds anchor. Native tracker corrects when quality is high.</summary>
			Anchored,

			/// <summary>Quality below lose threshold. Feeding anchor pose to native tracker for recovery.</summary>
			Recovery
		}
		...
		private class BodyState
		{
			...
			// Stationary mode phase
			public ARPhase Phase { get; set; }

			// Last known good pose (stored when quality >= nice, used as lerp target)
		...
		public int BodiesInARMode
		{
			get
			{
				...
				foreach (var state in _bodyStates.Values)
					if (state.Phase == ARPhase.Anchored || state.Phase == ARPhase.Recovery)
						count++;
				return count;
			}
		}
		...
		private void SubscribeToTrackingManager()
		{
			...
			if (manager != null)
			{
				manager.OnBeforeTrackingStep -= OnBeforeTrackingStep; // Prevent double subscribe
				manager.OnBeforeTrackingStep += OnBeforeTrackingStep;
		...
		/// <summary>
		/// Called before native tracking step.
		/// In Recovery phase: feeds current world pose to native tracker so it can re-acquire.
		/// The transform holds its position in world space (SLAM moves camera, not object).
		/// </summary>
		private void OnBeforeTrackingStep()
		{
#if HAS_AR_FOUNDATION
			if (!IsARPoseFusionActive)
				return;

			Transform cameraTransform = XRTrackerManager.Instance.CameraTransform;

			foreach ((TrackedBody body, BodyState state) in _bodyStates)
			{
				if (state.Phase != ARPhase.Recovery)
					continue;

				// Current world pose → camera-relative for native tracker
				Vector3 cameraRelativePos = cameraTransform.InverseTransformPoint(body.transform.position);
				Quaternion cameraRelativeRot = Quaternion.Inverse(cameraTransform.rotation) * body.transform.rotation;

				FTTrackingPose pose = ConversionUtils.GetConvertedPose(cameraRelativePos, cameraRelativeRot);
		...
		private void OnBeforeRender()
		{
			...
			if (manager.IsLicenseFrozen && !_wasLicenseFrozen)
			{
				...
				Debug.LogWarning("[FormulaTracker] Free license time limit reached. Tracking degraded. Upgrade to continue tracking.");
			}
...
#if HAS_AR_FOUNDATION
		/// <summary>
		/// Stationary mode state machine.
		/// Phases: None → Stabilizing → Anchored ↔ Recovery.
		/// SLAM holds position, tracker provides corrections when quality is good.
		/// </summary>
		private void ProcessStationary(TrackedBody body, BodyState state, float quality)
		{
			...
			{
				case ARPhase.None:
					state.Phase = ARPhase.Stabilizing;
					goto case ARPhase.Stabilizing;

				case ARPhase.Stabilizing:
					ProcessStabilizing(body, state);
					break;

				case ARPhase.Anchored:
					ProcessAnchored(body, state, quality);
					break;

				case ARPhase.Recovery:
					ProcessRecovery(body, state, quality);
					break;
			}
		}

		/// <summary>
		/// Stabilizing: apply native pose for STABILIZATION_FRAMES to establish world position.
		/// </summary>
		private void ProcessStabilizing(TrackedBody body, BodyState state)
		{
			ApplyNativePose(body);

			if (state.FramesSinceTrackingStarted >= TrackerDefaults.STABILIZATION_FRAMES)
			{
				state.Phase = ARPhase.Anchored;
				ResetCounters(state);
			}
		}

		/// <summary>
		/// Anchored: transform stays put (SLAM moves camera, not object).
		/// Lerps toward native pose as correction when quality is good.
		/// When quality drops below lose threshold → Recovery.
		/// </summary>
		private void ProcessAnchored(TrackedBody body, BodyState state, float quality)
		{
			float niceThreshold = NiceQuality(body);
			bool hasNativePose = GetNativeWorldPose(body, out Vector3 nativePos, out Quaternion nativeRot);

			// Store pose when quality is nice — this becomes the lerp target
			if (quality >= niceThreshold && hasNativePose)
			{
			...
			// Lerp toward last known good pose (even when current quality dips below nice)
			if (state.HasLastGoodPose)
			{
				float alpha = ComputeLerpAlpha(body.SmoothTime);
				body.SetWorldPose(
					Vector3.Lerp(body.transform.position, state.LastGoodPosition, alpha),
					Quaternion.Slerp(body.transform.rotation, state.LastGoodRotation, alpha));
			}

			// Check for quality drop → Recovery
			float loseThreshold = body.QualityToStopTracking;
			if (CheckFrameThreshold(state, quality < loseThreshold, TrackerDefaults.FRAMES_TO_LOSE, trackGoodFrames: false))
			{
				state.Phase = ARPhase.Recovery;
				ResetCounters(state);
			}
		}
		
		private void ProcessRecovery(TrackedBody body, BodyState state, float quality)
		{
			// Check AR reliability
			...
			// Check if tracker recovered → back to Anchored
			if (CheckFrameThreshold(state, quality >= body.QualityToStartTracking, StationaryFramesToRecover, trackGoodFrames: true))
			{
				state.Phase = ARPhase.Anchored;
				ResetCounters(state);
			}
		...
		/// <summary>
		/// Apply raw native tracker pose (camera-relative → world).
		/// </summary>
		private void ApplyNativePose(TrackedBody body)
		{
			if (FTBridge.FT_GetBodyPose(body.BodyId, out FTTrackingPose pose))
			{
				pose.GetTransformation(out Vector3 position, out Quaternion rotation);
				var cameraTransform = XRTrackerManager.Instance.CameraTransform;
				body.SetWorldPose(
					cameraTransform.TransformPoint(position),
					cameraTransform.rotation * rotation);
			}
		}

		/// <summary>
		/// Get native tracker pose converted to world space.
		/// </summary>
		private bool GetNativeWorldPose(TrackedBody body, out Vector3 worldPos, out Quaternion worldRot)
		{
			if (FTBridge.FT_GetBodyPose(body.BodyId, out FTTrackingPose pose))
			{
				pose.GetTransformation(out Vector3 localPos, out Quaternion localRot);
				var cam = XRTrackerManager.Instance.CameraTransform;
				worldPos = cam.TransformPoint(localPos);
				worldRot = cam.rotation * localRot;
				return true;
			}
		...
		{
			return _bodyStates.TryGetValue(body, out var state) &&
			       (state.Phase == ARPhase.Anchored || state.Phase == ARPhase.Recovery);
		}

		/// <summary>
		/// Get the current AR phase for a body (for debugging/UI).
		/// </summary>
		public string GetBodyARPhase(TrackedBody body)
		{
			if (_bodyStates.TryGetValue(body, out var state))
		...
		/// Called each frame by FormulaTrackingManager.
