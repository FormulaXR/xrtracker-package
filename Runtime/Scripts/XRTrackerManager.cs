...
namespace IV.FormulaTracker
{
	...
	public class XRTrackerManager : MonoBehaviour
	{
		...
		/// <summary>Current license tier (None = no license loaded, Free = 60s limit).</summary>
		...
		/// <summary>True if Free tier 60s limit has been reached.</summary>
		public bool IsLicenseFrozen => FTBridge.FT_IsLicenseFrozen();

		/// <summary>Seconds remaining for Free tier (60s). Returns -1 for non-Free tiers.</summary>
		public float FreeSecondsRemaining => FTBridge.FT_GetFreeSecondsRemaining();

		/// <summary>Machine ID used for Pro license node-locking.</summary>
		...
		private void LoadLicense()
		{
			...
			if (licenseJson == null)
			{
				...
				if (File.Exists(path))
				{
					...
					catch (Exception e) { Debug.LogWarning($"[FormulaTracker] Failed to read license: {e.Message}"); }
				}
			}

			// 4. No license found → generate Free
			if (licenseJson == null)
				licenseJson = GenerateFreeLicense();

			// Send to native
			if (string.IsNullOrEmpty(licenseJson)) return;
			...
			{
				Debug.Log($"[FormulaTracker] License: {_licenseTier}");
				CheckLicenseExpiryWarning();
			}
			else
				Debug.LogWarning($"[FormulaTracker] License failed: {_licenseStatus} (tier: {_licenseTier})");
		}
		...
		private void CheckLicenseExpiryWarning()
		{
			...
			if (days <= 30)
				Debug.LogWarning($"[FormulaTracker] License expires in {days} day{(days != 1 ? "s" : "")}. Check your email for the renewed license file.");
		}
		...
		private string FindLicenseInStreamingAssets()
		{
			...
			{
				Debug.LogWarning($"[FormulaTracker] Failed to read {licFiles[0]}: {e.Message}");
				return null;
			}
#endif
		}

		private static string GenerateFreeLicense()
		{
			...
			{
				Debug.LogWarning($"[FormulaTracker] Could not save Free license: {e.Message}");
			}
		...
		private void EnsureTrackedBodyManager()
		{
			if (TrackedBodyManager.Instance == null)
			{
				...
				Debug.Log("[FormulaTrackingManager] Auto-created TrackedBodyManager");
			}
		}
		...
		private async void Start()
		{
			if (_imageSource == ImageSource.Native)
			{
				...
				if (!string.IsNullOrEmpty(_autoSelectCameraName) && _availableCameras != null && _availableCameras.Length > 0)
				{
					...
					{
						Debug.LogWarning($"[FormulaTrackingManager] Camera '{_autoSelectCameraName}' not found, using first available camera");
						await SelectCameraAsync(0);
					}
					else
					{
						Debug.LogWarning($"[FormulaTrackingManager] Camera '{_autoSelectCameraName}' not found, no camera selected");
					}
				}
			}
		...
		public async Task SelectCameraAsync(int cameraIndex)
		{
			...
			{
				Debug.LogError($"[FormulaTrackingManager] Invalid camera index: {cameraIndex}");
				return;
			}
		...
		private async Task InitializeSequenceWithCoroutineAsync()
		{
			...
			{
				Debug.LogError("[FormulaTrackingManager] Sequence directory not set or invalid");
				return;
			}
		...
		private static string ResolveSequenceDirectory(string dir)
		{
			...
			Debug.Log($"[FormulaTrackingManager] No sequence in '{Path.GetFileName(dir)}', " +
			...
			FTCameraIntrinsics? depthIntrinsics, float depthScale)
		{
			...
			{
				Debug.LogError("[FormulaTrackingManager] InitializeInjectedAsync requires ImageSource.Injected");
				return false;
			}
			...
			if (result != 0)
			{
				Debug.LogError($"[FormulaTrackingManager] Failed to initialize injected mode (error: {result})");
				return false;
			}
		...
		public async Task<bool> InitializeRealSenseAsync()
		{
			...
			{
				Debug.LogError("[FormulaTrackingManager] InitializeRealSenseAsync requires ImageSource.RealSense");
				return false;
			}
			...
			if (resResult != 0 && resResult != FTErrorCode.NOT_SUPPORTED)
				Debug.LogWarning($"[FormulaTrackingManager] Failed to set RealSense resolution (error: {resResult})");

			int result = await InitTrackerAsync((int)ImageSource.RealSense, IntPtr.Zero);
			if (result != 0)
			{
				if (result == FTErrorCode.NOT_SUPPORTED)
					Debug.LogError("[FormulaTrackingManager] RealSense not supported - library not compiled with USE_REALSENSE=ON");
				return false;
			}
		...
		private async Task<int> InitTrackerAsync(int cameraMode, IntPtr cameraIntrinsics)
		{
			...
			if (result != 0)
			{
				Debug.LogError($"[FormulaTrackingManager] Failed to initialize (error: {result})");
				return result;
			}
		...
		private void LoadCalibrations()
		{
			...
			if (asset == null)
			{
				Debug.LogWarning("[FormulaTrackingManager] No calibration file assigned and no default found in Resources. Using default intrinsics.");
				_calibrations = null;
				return;
			}
			...
			{
				Debug.LogError($"[FormulaTrackingManager] Failed to parse calibrations: {e.Message}");
				_calibrations = null;
			}
		}
		...
		private CalibrationData FindCalibrationForDevice(string deviceName)
		{
			...
			Debug.LogWarning(
				$"[FormulaTrackingManager] No calibration found for camera '{deviceName}'. " +
		...
		public async Task InitializeWithCameraAsync(string cameraName, CalibrationIntrinsics intrinsics, CalibrationIntrinsics intrinsicsDist = null)
		{
			...
			if (index < 0)
			{
				Debug.LogError($"[FormulaTrackingManager] Camera '{cameraName}' not found");
				return;
			}
		...
		private void MatchCameraIntrinsics()
		{
			...
			{
				Debug.LogWarning("[FormulaTrackingManager] FT_GetCameraIntrinsics returned invalid data, skipping projection update");
				return;
			}
		...
		private static void OnLogMessage(int logLevel, string message)
		{
			switch (logLevel)
			{
				case 0: Debug.Log($"[FormulaTracker] {message}"); break;
				case 1: Debug.LogWarning($"[FormulaTracker] {message}"); break;
				case 2: Debug.LogError($"[FormulaTracker] {message}"); break;
			}
		}
